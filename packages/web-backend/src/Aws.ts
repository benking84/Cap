...
export class AwsCredentials extends Effect.Service<AwsCredentials>()(
	"AwsCredentials",
	{
		effect: Effect.gen(function* () {
			const accessKeyId = yield* Config.string("CAP_AWS_ACCESS_KEY").pipe(
				Config.option,
			);
			const secretAccessKey = yield* Config.string("CAP_AWS_SECRET_KEY").pipe(
				Config.option,
			);
			...
			} else if (Option.isSome(accessKeyId) && Option.isSome(secretAccessKey)) {
